...
for i in range(10):
  ...
  Pred_dataloaders_w_zeros.append(numbersets)

print(len(Pred_dataloaders_w_zeros))
print(len(Pred_dataloaders_w_zeros[0]))
...
optimizerPredictor = optim.Adam(predModel.parameters(), lr=1e-3)
...
epoch_loss_scores = np.zeros((10,11,num_epochs))

torch.save(model.state_dict(), './vae_random.pth')
torch.save(predModel.state_dict(), './pred_random.pth')

#loop through each number
for number in range(1,10):
    #train VAE on value and numbers
    print('Training VAE for number {}'.format(number))
    """Training VAE"""
    ...
    for epoch in range(num_epochs):
        train_loss = 0
        for batch_idx, data in enumerate(VAE_dataloaders_w_zeros[number]):
            img, _ = data
            img = img.view(img.size(0), -1)
            img = Variable(img)
            if torch.cuda.is_available():
            ...
            #        epoch,
            #        batch_idx * len(img),
            #        len(VAE_dataloaders_w_zeros[number].dataset), 100. * batch_idx / len(VAE_dataloaders_w_zeros[number]),
            #        loss.item() / len(img)))

        #print('===={}=====> Epoch: {} Average loss: {:.4f}'.format(
        #    number, epoch, train_loss / len(VAE_dataloaders_w_zeros[number].dataset)))
        if epoch % 20 == 0:
            save = to_img(recon_batch.cpu().data)
            save_image(save, './vae_img/network_{}_image_{}.png'.format(number, epoch))
    #model.train(mode=False)
    #for param in model.parameters():
    #    param.requires_grad = False
    torch.save(model.state_dict(), './vae_{}.pth'.format(number))

    #loop through each number
    for percent in range(11):
        print('Training predictor for number {} and percent {}'.format(number, percent))
        predModel.load_state_dict(torch.load( './pred_random.pth'))
        optimizerPredictor = optim.Adam(predModel.parameters(), lr=1e-3)
        #train predictor
        ...
        for epoch in range(num_epochs):
            train_loss = 0
            for batch_idx, data in enumerate(Pred_dataloaders_w_zeros[number][percent]):
                img, _ = data
                img = img.view(img.size(0), -1)
                img = Variable(img)
                if torch.cuda.is_available():
                    img = img.cuda()
                optimizerPredictor.zero_grad()
                mu, logvar = model.encode(img)
                modelOutput = model.reparametrize(mu, logvar)
                ...
                predLoss.backward()
                train_loss += predLoss.item()
                optimizerPredictor.step()
                #if batch_idx % 100 == 0:
                #    print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                #        epoch,
                #        batch_idx * len(img),
                #        len(Pred_dataloaders_w_zeros[number][percent].dataset), 100. * batch_idx / len(Pred_dataloaders_w_zeros[number][percent]),
                #        predLoss.item() / len(img)))

            #print('===={}=====> Epoch: {} Average loss: {:.4f}'.format(
            #    number, epoch, train_loss / len(Pred_dataloaders_w_zeros[number][percent].dataset)))
            epoch_loss_scores[number, percent, epoch] = train_loss / len(Pred_dataloaders_w_zeros[number][percent].dataset)
            if epoch % 10 == 0:
                save = to_img(recon_batch.cpu().data)
                save_image(save, './vae_img_pred/network_{}_percent_{}_image_{}.png'.format(number, percent, epoch))
                save_truth = to_img(recon_batch_truth.cpu().data)
                save_image(save_truth, './vae_img_pred/network_{}_percent_{}_image_{}_truth.png'.format(number, percent, epoch))
        #predModel.train(mode=False)
        torch.save(predModel.state_dict(), './vaePred_{}_{}.pth'.format(number, percent))
        
        print('Testing Predictor for number {} and percent {}'.format(number,percent))
        ...
        print('====> Average Test loss: {:.4f}'.format(
            train_loss / len(dataloader.dataset)))
        loss_scores[number,percent] = train_loss / len(dataloader.dataset)
...
np.save('epoch_loss_scores.npy', epoch_loss_scores)

print(loss_scores)
print(epoch_loss_scores)
